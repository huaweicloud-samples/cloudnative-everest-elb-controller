...
`cloudnative-everest-elb-controller` 是一个 Kubernetes 控制器，为 [OpenEverest](https://openeverest.io/documentation/current/) 数据库集群自动创建和管理**华为云 ELB**（弹性负载均衡）实例。
...
本项目基于 MIT-0 许可证（MIT No Attribution）开源。详见 [LICENSE](LICENSE)。

---

## 联系方式 / 维护者

- **仓库地址**：[cloudnative-everest-elb-controller](https://github.com/huaweicloud-samples/cloudnative-everest-elb-controller)
- **所属组织**：[huaweicloud-samples](https://github.com/huaweicloud-samples)
- **问题反馈**：[GitHub Issues](https://github.com/huaweicloud-samples/cloudnative-everest-elb-controller/issues)
- **邮箱**：huaweicloud-samples@huaweicloud.com
